...
sub _validate_with_plugin {
    ...
    for my $row ( 1 .. $row_max ) {
        ...
        #tissue_type must not be blank and must be either needle, cambium, leaf, root, stem, seed, fruit, tuber
        if (!$tissue_type || $tissue_type eq '' || ($tissue_type ne 'needle' && $tissue_type ne 'cambium' && $tissue_type ne 'leaf' && $tissue_type ne 'root' && $tissue_type ne 'stem' && $tissue_type ne 'seed' && $tissue_type ne 'fruit' && $tissue_type ne 'tuber')) {
            push @error_messages, "Cell E$row_name: column tissue type and must be either needle, cambium, stem, leaf, root, seed, fruit or tuber";
        }
