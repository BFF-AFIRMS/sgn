...
my @OPTIONAL_COLUMNS = qw|intercrop_accession_name plot_name trial_type trial_stock_type plot_width plot_length field_size planting_date transplanting_date harvest_date is_a_control rep_number range_number row_number col_number stake_number set_number seedlot_name num_seed_per_plot weight_gram_seed_per_plot entry_number|;
...
sub _parse_with_plugin {
    ...
    for my $row (@$data) {
        ...
        my $stake_number = $row->{'stake_number'};
        my $set_number = $row->{'set_number'};
        ...
        if ($stake_number) {
            $design_details{$key}->{stake_number} = $stake_number;
        }
        if ($set_number) {
            $design_details{$key}->{set_number} = $set_number;
        }
        if ($seedlot_name){
