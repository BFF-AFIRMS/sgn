...
sub retrieve_plot_info {
    ...
    my $stake_prop = $stockprop_hash{$self->cvterm_id('stake')} ? join ',', @{$stockprop_hash{$self->cvterm_id('stake')}} : undef;
    my $set_prop = $stockprop_hash{$self->cvterm_id('set')} ? join ',', @{$stockprop_hash{$self->cvterm_id('set')}} : undef;
    ...
    if ($stake_prop) {
        $design_info{"stake"}=$stake_prop;
    }
    if ($set_prop) {
        $design_info{"set"}=$set_prop;
    }
    if ($self->get_experiment_type eq 'genotyping_layout'){
	if ($is_blank_prop) {
