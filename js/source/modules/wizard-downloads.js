...
export function WizardDownloads(main_id,wizard){
  ...
  wizard.on_change((c,s,o)=>{
    ...
    var subplots = categories.indexOf("subplots")!=-1 ? selections["subplots"] : [];
    ...
    main.selectAll(".wizard-download-phenotypes")
      ...
      .on("click",()=>{
        ...
        var subplot_ids = JSON.stringify(subplots.map(d=>d.id));
        ...
        var min = d3.select(".wizard-download-phenotypes-min").node().value;
	      ...
	      var missing_format = d3.select(".wizard-download-phenotypes-missing-format").node().value;
